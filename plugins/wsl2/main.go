...
// provider holds the plugin's mutable state, set up in initialize and read by
// the handler methods. tsAPIKey is the Tailscale API key (set once in
...
func (p *provider) initialize(ctx context.Context, params sdk.InitializeParams) (sdk.InitializeResult, error) {
	...
	if _, err := sshhost.Run(ctx, p.sshClient, "grep -qi microsoft /proc/version"); err != nil {
		return sdk.InitializeResult{}, sdk.Errf(sdk.CatValidation,
			"wsl2.host.not_wsl2",
			"ssh_host %s is not a WSL2 environment (/proc/version has no 'microsoft' marker); use the qemu plugin for a plain Linux host", p.cfg.SSHHost)
	}
	...
	if _, err := sshhost.Run(ctx, p.sshClient, "test -e /dev/kvm"); err != nil {
		return sdk.InitializeResult{}, sdk.Errf(sdk.CatValidation,
			"wsl2.host.no_kvm",
			"/dev/kvm is not present on WSL2 host %s: enable nested virtualization (nestedVirtualization=true in .wslconfig) and ensure the CPU supports VT-x/AMD-V", p.cfg.SSHHost)
	}
