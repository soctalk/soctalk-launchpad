// launchpad-plugin-wsl2 provisions VMs "on Windows (WSL2)": it SSHes into a
// WSL2 Ubuntu environment running on a Windows host and drives
// qemu-system-x86_64 there — a full VM under nested KVM inside WSL2 — with a
// cloud-init seed ISO. The VMs join a Tailscale tailnet at first boot, and the
// plugin returns the assigned Tailscale IPv4 as the primary contact address.
//
// The SSH target must be a genuine WSL2 distro (verified via /proc/version) with
// nested virtualization enabled so /dev/kvm is present; otherwise qemu cannot
// accelerate and the guest will not boot in a usable time.
//
// Config (params.config):
//   ssh_host:     ops@100.102.223.8    (Tailscale IP or hostname of the WSL2 distro)
...
func main() {
	...
	if err != nil {
		fmt.Fprintln(os.Stderr, "wsl2 plugin:", err)
		os.Exit(1)
	}
}
...
func initialize(ctx context.Context, params sdk.InitializeParams) (sdk.InitializeResult, error) {
	...
	if tsAPIKey == "" {
		return sdk.InitializeResult{}, sdk.Errf(sdk.CatAuth,
			"wsl2.tailscale.missing_api_key",
	...
	if _, err := runOverSSH(ctx, sshClient, probe); err != nil {
		return sdk.InitializeResult{}, sdk.Errf(sdk.CatValidation,
			"wsl2.host.missing_tools",
			"host is missing qemu-system-x86_64 / genisoimage / qemu-img / curl: %v", err)
	}
	// WSL2-awareness probe: this plugin exists to run VMs *inside* a WSL2
	// Ubuntu environment on a Windows host, so verify the SSH target really is
	// WSL2. WSL kernels carry "microsoft" in /proc/version (e.g.
	// "Linux ... microsoft-standard-WSL2 ..."). If it's absent the operator has
	// almost certainly pointed ssh_host at a plain Linux box — use the qemu
	// plugin for that instead.
	if _, err := runOverSSH(ctx, sshClient, "grep -qi microsoft /proc/version"); err != nil {
		return sdk.InitializeResult{}, sdk.Errf(sdk.CatValidation,
			"wsl2.host.not_wsl2",
			"ssh_host %s is not a WSL2 environment (/proc/version has no 'microsoft' marker); use the qemu plugin for a plain Linux host", cfg.SSHHost)
	}
	// KVM probe: WSL2 exposes /dev/kvm only when nested virtualization is
	// enabled (nestedVirtualization=true in .wslconfig, on a host CPU with
	// VT-x/AMD-V). Without it qemu-system-x86_64 -enable-kvm cannot start.
	if _, err := runOverSSH(ctx, sshClient, "test -e /dev/kvm"); err != nil {
		return sdk.InitializeResult{}, sdk.Errf(sdk.CatValidation,
			"wsl2.host.no_kvm",
			"/dev/kvm is not present on WSL2 host %s: enable nested virtualization (nestedVirtualization=true in .wslconfig) and ensure the CPU supports VT-x/AMD-V", cfg.SSHHost)
	}
...
func plan(ctx context.Context, params sdk.VMPlanParams, emit sdk.Emitter) (sdk.VMPlanResult, error) {
	return sdk.VMPlanResult{
		Summary: fmt.Sprintf("wsl2: %s on %s (%dvCPU/%dMB, tailscale hostname lp-%s)",
...
func create(ctx context.Context, params sdk.VMCreateParams, emit sdk.Emitter) (sdk.VMCreateResult, error) {
	...
	if alive, _ := pidAliveOverSSH(ctx, sshClient, pidPath); alive {
		...
		_, _ = runOverSSH(ctx, sshClient, "kill $(cat "+shellEscape(pidPath)+") 2>/dev/null || true")
	...
	if _, err := runOverSSH(ctx, sshClient,
		...
		return sdk.VMCreateResult{}, sdk.Errf(sdk.CatInternal,
			"wsl2.qemu-img.create_failed", "%v", err)
	}
  ...
  -daemonize > %s/nohup.out 2>&1`,
		cfg.MemoryMB, cfg.CPU,
		shellEscape(work), shellEscape(work), shellEscape(work),
		shellEscape(pidPath), shellEscape(work),
	)
	if _, err := runOverSSH(ctx, sshClient, launch); err != nil {
		return sdk.VMCreateResult{}, sdk.Errf(sdk.CatInternal,
			"wsl2.qemu.launch_failed", "%v", err)
	}
...
func destroy(ctx context.Context, params sdk.VMDestroyParams, emit sdk.Emitter) (sdk.VMDestroyResult, error) {
	...
	out, _ := runOverSSH(ctx, sshClient, fmt.Sprintf(
		...
		shellEscape("qemu-system.*"+work), shellEscape("qemu-system.*"+work),
	))
...
func inspect(ctx context.Context, params sdk.VMInspectParams, emit sdk.Emitter) (sdk.VMInspectResult, error) {
	...
	work := vmWorkDir(params.RunID, params.VMKey)
	pidPath := work + "/qemu.pid"
	alive, _ := pidAliveOverSSH(ctx, sshClient, pidPath)
	hostname := hostnameFor(params.VMKey)
	ipv4 := ""
	state := "unknown"
	if device, _ := findTailscaleDevice(ctx, cfg.Tailnet, hostname); device != nil {
...
func mintTailscaleKey(ctx context.Context, tailnet, tag string) (string, error) {
	...
	if err != nil {
		return "", sdk.Errf(sdk.CatProviderUnavailable,
			"wsl2.tailscale.api_error", "%v", err)
	}
	defer resp.Body.Close()
	raw, _ := io.ReadAll(resp.Body)
	if resp.StatusCode >= 400 {
		return "", sdk.Errf(sdk.CatAuth,
			"wsl2.tailscale.api_status",
	...
	if out.Key == "" {
		return "", sdk.Errf(sdk.CatInternal,
			"wsl2.tailscale.key_missing", "no key in tailscale response: %s", string(raw))
	}
	return out.Key, nil
}
