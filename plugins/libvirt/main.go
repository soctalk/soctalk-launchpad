// launchpad-plugin-libvirt provisions VMs on a remote libvirt/KVM host over
// SSH using virt-install + virsh (qemu:///session) with a cloud-init seed ISO. The VMs join a
...
type config struct {
	...
	Network   string   `json:"network,omitempty"` // libvirt network (default: default)
}
...
func main() {
	...
	if err != nil {
		fmt.Fprintln(os.Stderr, "libvirt plugin:", err)
		os.Exit(1)
	}
}
...
func initialize(ctx context.Context, params sdk.InitializeParams) (sdk.InitializeResult, error) {
	...
	if cfg.SSHHost == "" || cfg.WorkDir == "" || cfg.Tailnet == "" {
		return sdk.InitializeResult{}, sdk.Errf(sdk.CatValidation,
			"libvirt.config.incomplete",
			"ssh_host, work_dir, tailnet are all required")
	}
	if cfg.Network == "" {
		cfg.Network = "default" // libvirt's built-in NAT network
	}
	...
	if tsAPIKey == "" {
		return sdk.InitializeResult{}, sdk.Errf(sdk.CatAuth,
			"libvirt.tailscale.missing_api_key",
	...
	if err != nil {
		return sdk.InitializeResult{}, sdk.Errf(sdk.CatProviderUnavailable,
			"libvirt.ssh.unreachable",
			"cannot SSH to %s: %v (agent-loaded key expected)", cfg.SSHHost, err)
	}
	sshClient = c
	// Quick probe: libvirt tooling (virsh/virt-install), seed-ISO + qcow2 tools.
	probe := "which virsh && which virt-install && which genisoimage && which qemu-img"
	if cfg.BaseImage == "" {
		probe += " && which curl"
	}
	if _, err := runOverSSH(ctx, sshClient, probe); err != nil {
		return sdk.InitializeResult{}, sdk.Errf(sdk.CatValidation,
			"libvirt.host.missing_tools",
			"host is missing virsh / virt-install / genisoimage / qemu-img / curl: %v", err)
	}
...
func plan(ctx context.Context, params sdk.VMPlanParams, emit sdk.Emitter) (sdk.VMPlanResult, error) {
	return sdk.VMPlanResult{
		Summary: fmt.Sprintf("libvirt: %s on %s (%dvCPU/%dMB, tailscale hostname lp-%s)",
...
// domainName is the libvirt domain name for a VM — unique per run so repeated
// runs don't collide. virsh runs against the per-user session connection
// (qemu:///session) so no sudo is needed; networking is usermode NAT, matching
// the tailnet-overlay model.
func domainName(runID, vmKey string) string {
	return "lp-" + sanitizeHostname(runID+"-"+vmKey)
}

const virshConn = "virsh --connect qemu:///session"

// domainRunning reports whether the libvirt domain is defined and running.
func domainRunning(ctx context.Context, dom string) bool {
	out, err := runOverSSH(ctx, sshClient, virshConn+" domstate "+shellEscape(dom)+" 2>/dev/null")
...
func create(ctx context.Context, params sdk.VMCreateParams, emit sdk.Emitter) (sdk.VMCreateResult, error) {
	if sshClient == nil {
		return sdk.VMCreateResult{}, sdk.Errf(sdk.CatAuth,
			"libvirt.not_initialized",
			"plugin.initialize has not been called successfully")
	}
	spec := params.Spec

	work := vmWorkDir(spec.RunID, spec.VMKey)
	dom := domainName(spec.RunID, spec.VMKey)

	// Idempotency: reuse only when the domain is running AND its tailnet device
	// is online. A domain whose device was revoked is unreachable forever
	// (usermode NAT means the tailnet is the only path in), so destroy + reprovision.
	emit.Progress("lookup", 3, "checking for existing domain")
	if domainRunning(ctx, dom) {
		d, _ := findTailscaleDevice(ctx, cfg.Tailnet, hostnameFor(spec.VMKey))
		if d != nil && d.online() {
			emit.Log("info", "reusing running libvirt domain (tailnet device online)",
				map[string]any{"domain": dom, "ipv4": d.primaryIPv4()})
			return currentResult(ctx, spec, emit)
		}
		emit.Log("warn", "existing libvirt domain has no live tailnet device — destroying and re-provisioning",
			map[string]any{"domain": dom})
		_, _ = runOverSSH(ctx, sshClient, virshConn+" destroy "+shellEscape(dom)+" 2>/dev/null; "+virshConn+" undefine "+shellEscape(dom)+" 2>/dev/null || true")
	...
	if _, err := runOverSSH(ctx, sshClient,
		fmt.Sprintf("qemu-img create -f qcow2 -F qcow2 -b %s %s/disk.qcow2 %dG",
			shellEscape(baseImagePath), shellEscape(work), cfg.DiskGB)); err != nil {
		return sdk.VMCreateResult{}, sdk.Errf(sdk.CatInternal,
			"libvirt.qemu-img.create_failed", "%v", err)
	}

	// Define + start the libvirt domain via virt-install --import.
	//
	// Uses qemu:///session (per-user, no sudo) with usermode NAT networking so
	// the guest reaches the internet to install Tailscale + apt; Tailscale then
	// becomes the overlay path in. --import boots the existing disk (no OS
	// install), --noautoconsole returns immediately.
	emit.Progress("boot", 75, "defining + starting libvirt domain")
	launch := fmt.Sprintf(`virt-install --connect qemu:///session \
  --name %s \
  --memory %d --vcpus %d \
  --disk path=%s/disk.qcow2,format=qcow2,bus=virtio \
  --disk path=%s/seed.iso,device=cdrom \
  --os-variant ubuntu24.04 \
  --network user,model=virtio \
  --graphics none --noautoconsole --import`,
		shellEscape(dom), cfg.MemoryMB, cfg.CPU,
		shellEscape(work), shellEscape(work),
	)
	if out, err := runOverSSH(ctx, sshClient, launch); err != nil {
		return sdk.VMCreateResult{}, sdk.Errf(sdk.CatInternal,
			"libvirt.virt_install.failed", "%v: %s", err, out)
	}

	emit.Progress("boot", 95, "domain started; tailscale + cloud-init running inside")
...
func destroy(ctx context.Context, params sdk.VMDestroyParams, emit sdk.Emitter) (sdk.VMDestroyResult, error) {
	...
	dom := domainName(params.RunID, params.VMKey)
	...
	emit.Progress("destroy", 30, "destroying + undefining libvirt domain")
	// virsh destroy stops a running domain (rc 0), undefine removes its config.
	// Echo markers tell us whether the domain actually existed.
	out, _ := runOverSSH(ctx, sshClient, fmt.Sprintf(
		"if %s destroy %s 2>/dev/null; then echo destroyed; fi; if %s undefine %s --nvram 2>/dev/null; then echo undefined; fi",
		virshConn, shellEscape(dom), virshConn, shellEscape(dom)))
	if strings.Contains(out, "destroyed") || strings.Contains(out, "undefined") {
		didWork = true
	}
...
func mintTailscaleKey(ctx context.Context, tailnet, tag string) (string, error) {
	...
	if err != nil {
		return "", sdk.Errf(sdk.CatProviderUnavailable,
			"libvirt.tailscale.api_error", "%v", err)
	}
	defer resp.Body.Close()
	raw, _ := io.ReadAll(resp.Body)
	if resp.StatusCode >= 400 {
		return "", sdk.Errf(sdk.CatAuth,
			"libvirt.tailscale.api_status",
	...
	if out.Key == "" {
		return "", sdk.Errf(sdk.CatInternal,
			"libvirt.tailscale.key_missing", "no key in tailscale response: %s", string(raw))
	}
	return out.Key, nil
}
