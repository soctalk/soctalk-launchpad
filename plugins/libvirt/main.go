...
// provider owns the plugin's mutable state for one plugin process.
type provider struct {
	cfg       config
	sshClient *ssh.Client
	// tsAPIKey is the Tailscale API key. Set once in initialize.
	tsAPIKey string
	// imageCache memoizes the resolved base-image path on the target host.
...
func (p *provider) initialize(ctx context.Context, params sdk.InitializeParams) (sdk.InitializeResult, error) {
	...
	if p.cfg.Network == "" {
		p.cfg.Network = "default" // libvirt's built-in NAT network
	}
...
func (p *provider) domainRunning(ctx context.Context, dom string) bool {
	out, err := sshhost.Run(ctx, p.sshClient, virshConn+" domstate "+sshhost.ShellEscape(dom)+" 2>/dev/null")
...
func (p *provider) create(ctx context.Context, params sdk.VMCreateParams, emit sdk.Emitter) (sdk.VMCreateResult, error) {
	...
	if p.domainRunning(ctx, dom) {
		...
		_, _ = sshhost.Run(ctx, p.sshClient, virshConn+" destroy "+sshhost.ShellEscape(dom)+" 2>/dev/null; "+virshConn+" undefine "+sshhost.ShellEscape(dom)+" 2>/dev/null || true")
  ...
  --graphics none --noautoconsole --import`,
		sshhost.ShellEscape(dom), p.cfg.MemoryMB, p.cfg.CPU,
		sshhost.ShellEscape(work), sshhost.ShellEscape(work),
	)
	if out, err := sshhost.Run(ctx, p.sshClient, launch); err != nil {
		return sdk.VMCreateResult{}, sdk.Errf(sdk.CatInternal,
...
func (p *provider) destroy(ctx context.Context, params sdk.VMDestroyParams, emit sdk.Emitter) (sdk.VMDestroyResult, error) {
	...
	out, _ := sshhost.Run(ctx, p.sshClient, fmt.Sprintf(
		...
		virshConn, sshhost.ShellEscape(dom), virshConn, sshhost.ShellEscape(dom)))
